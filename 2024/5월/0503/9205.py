...
def bfs(n, points, visited):
    q = deque()
    q.append([points[0], 20, visited]) # 집에서 시작

    # 모든 경로에 대해 고려해줘야 하므로 루트별로 visited 판별 필요
    
    while q:
        now_point, now_beer, now_visited = q.popleft() # 현재 위치와 남은 맥주의 수
        if now_point == points[-1]:
            return 'happy'
        
        for i in range(n + 1):
            next_point = points[i + 1] # 편의점 위치, 페스티벌 장소
            length = abs(now_point[0] - next_point[0]) + abs(now_point[1] - next_point[1])
            
            if length % 50 == 0:
                req_beer = length // 50
            else:
                req_beer = length // 50 + 1
            
            if now_beer >= req_beer and now_visited[i] == 0:
                now_visited[i] = 1
                q.append([next_point, 20, now_visited])
...
for _ in range(t):
    ...
    points = [list(map(int, input().split())) for _ in range(n + 2)] # points[0]은 집, points[-1]은 락페스티벌 좌표, 그 외는 편의점
    
    visited = [0] * (n + 1) # 편의점, 락페
    answer = bfs(n, points, visited)
